...
const extractText = async (file) => {
  ...
  console.log(
    `Extracted text from ${filePath}: ${content.substring(0, 100)}...`,
  );
  return content;
};
...
router.post(
  ...
  async (req, res) => {
    ...
    if (!files || !files.sop || !files.transcript || !files.cv) {
      ...
      return res
        .status(400)
        .json({ error: "Missing one or more files (sop, transcript, cv)" });
    }
...
Transcript (Academic Record): ${transcriptText}`;
      ...
      try {
        ...
        predictionData = {
          predicted_score: 55,
          reasoning:
            "The AI model's response could not be parsed into the required JSON format. This might be due to a formatting error in the AI output. A default score of 55 has been assigned. Please try again or check the AI provider's status.",
        };
      }
      ...
      try {
        await admin.firestore().runTransaction(async (tx) => {
          ...
          const freshPlan = String(
            getEffectivePlan(freshUserData, {
              defaultPlan: "free",
              trialPlan: "pro",
            }),
          ).toLowerCase();
          ...
          const predictionsCollection = admin
            .firestore()
            .collection("predictions");
        ...
        return res
          .status(500)
          .json({ error: "Internal server error while saving prediction" });
      }
      ...
      res
        .status(500)
        .json({ error: "Internal server error during prediction process" });
    }
  },
);
